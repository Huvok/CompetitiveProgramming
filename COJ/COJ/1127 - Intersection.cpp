...
void toStandard(Point<double> p1, Point<double> p2, double &a, double &b, double &c)
{
...
Line<double> toParametric(double a, double b, double c)
{
	double y1 = -c / b;
	double x1 = (-c - y1 * b) / a;
...
int main()
{
	...
	while (n--)
	{
		...
		Point<double> pbl(xlt, yrb), pbr(xrb, yrb), ptl(xlt, ylt), ptr(xrb, ylt);
		Line<double> l1(pbl, pbr), l2(pbr, ptr), l3(ptr, ptl), l4(ptl, pbl);
		...
		Line<double> dummy;
		if (intersectSegments(line, l1, dummy) ||
			intersectSegments(line, l2, dummy) ||
			intersectSegments(line, l3, dummy) ||
			intersectSegments(line, l4, dummy) ||
			pointVsPolygon(a, poly) == -1 ||
			pointVsPolygon(b, poly) == -1)
			cout << "T" << endl;
		else
			cout << "F" << endl;
	}
