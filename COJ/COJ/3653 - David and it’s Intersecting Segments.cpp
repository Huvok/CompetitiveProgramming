...
int main()
{
	...
	FOR(i, 0, sz(hor))
	{
		FOR(j, 0, sz(ver))
		{
			if (intersectSegments(hor[i], ver[j], res))
			{
				adj[i + 1].pb(sz(hor) + j + 1);
				adj[sz(hor) + j + 1].pb(i + 1);
			}
		}
	}

	cout << tot - hopcroftKarp() << endl;
